...
# ---------------------------------------------------------------------------
# Direct CZI read path
# ---------------------------------------------------------------------------


def test_czi_fast_read_ok_detection() -> None:
    """Fast path is enabled only for (non-BGR) CZI sources."""
    assert OmeZarrConverter._czi_fast_read_ok(
        str(LOCAL_RESOURCES_DIR / "s_3_t_1_c_3_z_5.czi")
    )
    assert not OmeZarrConverter._czi_fast_read_ok(
        str(LOCAL_RESOURCES_DIR / "s_3_t_1_c_3_z_5.ome.tiff")
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "s_3_t_1_c_3_z_5.czi",
            [
                (slice(0, 3), slice(0, 5), slice(0, 325), slice(0, 475)),  # full
                (slice(1, 3), slice(2, 4), slice(0, 325), slice(0, 475)),  # part C/Z
                (slice(0, 2), slice(0, 5), slice(50, 200), slice(100, 400)),  # part YX
            ],
        ),
        # CYX (1, 325, 475)
        (
            "s_1_t_1_c_1_z_1.czi",
            [
                (slice(0, 1), slice(0, 325), slice(0, 475)),  # full
                (slice(0, 1), slice(10, 300), slice(20, 460)),  # part YX
            ],
        ),
    ],
    ids=["czyx", "cyx"],
)
def test_read_region_czi_matches_dask(
    filename: str,
    regions: List[Tuple[slice, ...]],
) -> None:
    """Direct held-open CZI read is byte-identical to the dask reader path."""
    src = str(LOCAL_RESOURCES_DIR / filename)
    bio = BioImage(src)
    bio.set_scene(0)
    order = bio.reader.dims.order.upper()
    dtype = bio.reader.dtype
    dask_data = bio.reader.get_image_dask_data(order)

    for region in regions:
        expected = dask_data[region].compute()
        actual = OmeZarrConverter._read_region_czi(src, region, order, 0, dtype)
        assert_array_equal(
            actual,
            expected,
            err_msg=f"{filename} region {region} differs from dask read",
        )
