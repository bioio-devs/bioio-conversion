...
class OmeZarrConverter:
    """
    Convert any BioImage-supported format (TIFF, CZI, etc.) into OME-Zarr.

    Supports exporting one, many, or all scenes from a multi-scene file.

    **Zarr v3 defaults** (when ``zarr_format=3`` and no explicit
    ``level_shapes``, ``num_levels``, ``chunk_shape``, or ``shard_shape``):

    - *Pyramid*: levels are generated by ``_build_pyramid_shapes``, halving
      X and Y (and later Z) until all Z slices of the smallest level tile into
      a 2048 × 2048 atlas canvas.
    - *Chunks*: viewer-optimised layout — whole XY planes per chunk, T=1,
      C=1; Z grows to fill ``memory_target`` (default 16 MiB).
    - *Shards*: chunks are packed along Z → C → T up to 4 GiB per shard.

    Setting ``num_levels`` bypasses the atlas-based pyramid and uses a simple
    XY half-pyramid instead.  Setting ``chunk_shape`` or ``shard_shape``
    overrides auto-computation entirely.
    """
